...
def rank_scored(catalog: Catalog, state: SessionState, limit: int) -> list[tuple[float, str]]:
    ...
    pool = candidates(catalog, state, ctx)
    ...
    # Instrumentation only — records how far each turn narrowed the field so the
    # funnel can be inspected and demonstrated. Nothing here affects ranking.
    committed = committed_category(state)
    state.funnel.append({
        "turn": state.turn,
        "catalog": len(catalog),
        ...
        "ruled_out": len(excluded),
        "type": state.frame.type_phrase,
        "category": committed,
        "bucket": len(catalog.by_category.get(committed, ())) if committed else None,
        "constraints": len(state.constraints),
        "top_score": round(scored[0][0], 2) if scored else None,
    })
